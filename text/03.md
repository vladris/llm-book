...
**ID: I think I'd sidebar early a few points that are easy to forget 1/non-deterministic in general 2/non-determinstic based on input
3/ Art, not science
4/ There's an idea in here about prompt 'response' evaluation. LLMs need good prompts because it's not clear what the success criteria is. How does our intern know 'what you want'*
...
**ID: Who is we here? the industry, or us in this example?**
...
* **How the output should look like** – You can be as descriptive as needed.
  ...
  *each”.
    **(ID: 'How the ouptut should look ' feels ackward, maybe 'What the output should look like', I like how later you're using shape of the output?)

* **How to go about generating the output** – Additional instructions for the
  *large language model, like “think step by step”.
    **(ID: 'Forward reference to why this important?')
...
**(ID: 'Integration w/other systems', doesn't feel right if output is for a human), this example only uses one of your points above, not sure if tha'ts deliberate**
...
**ID> I've never heard or super prompts - LOVE IT**

**ID> Maybe talk about these being emergent/unxpected behaviors, that are being discovered, and don't feel bad if you can't figure out a super prompt on your own*


**ID> There's something meta here around prompts are model dependant **

**ID> Maybe something like there is no 'perfect prompt', and due to non-deteriminism, even awesome prompts will have poor performance
**
...
**ID> Why does chain of thoughts prompting matter? I've seen stuff around you have only so much computation per output token.

See build talk -- not sure where that should go. Other important thing, is the forward nature of GPTs, lik even if it knows it's wrong it can't stop once it's committed**
...
**ID> I'd stay on turbo the whole time, switching models is confusing. Find a harder examplet hat works on turbo**
...
**ID> In other cases simply -> Another technique is to**
...
language models on logic, math, and symbolic reasoning tasks[^2]. Often adding a small phrase, or examples dramatically improve the performance of a prompt. While large language models can
...
and identify the right way to ask questions to get the best responses. **ID>Move this up**
...
**ID> Oh do you talk about plugins in the book - love the wolfram quote. LLMs are bad at math,  so are humans, so we gave them calculators, that's plugins**
...
**ID> Maybe say something like, we design the prompt statically, but the intent is to merge with input, user or otherwise,  this is where templates come in

META:  I saw this somehwere, need to massage and maybe put early-- The difference btween conventional programing and LLM programming the inputs are locked and you write the algorithm.  In ML you write the inputs and the algorithm is locked.

Hymn, maybe you need a table of the difference between LLMs and programming,  with ach difference and the consequence

Oh, you go into this in implementation details

(Feels like I'm writing notes to myself as well, feel free to ignore)
**
...
**ID> This should go somewhere else, but thinking about tokens ... What'st he value in setting max tokens beyond exceeding aswer length? How do you detect your answers is cut off? Maybe that's the key, ensuring your answer isn't cut off, which you detect if you hit max tokens, though hard to figure out what to do then ...**
...
**ID> Thinking about cost, for GPT3.5, does it matter**

**ID> Do you have a chapter on 'static testing' and production modelling*

Figure 3.1 shows how prompt templating works.

![Figure 3.1](images/03/fig1.png)

**ID> Took me a while to parse this diagram, you are listing the nouns in the picture ... I'm probably over thinking it, but the verb to instanitate the prompt might be nice ...
...
**ID>   Some of this feedback goes back to introducing the prompt templatef.

- So, this idea that a prompt template is a prompt + model params + model is interesting. Need to talk about that explicility. Perhaps include model in the prompt tempalte then? Though this adds complexity with chat model vs text model.... I'd expect to split prompt creation from prompt execution.... Maybe need a sidebar on how you're using simplified examples, but in general would split them... Or maybe you wouldn't.

E.g. 1/ Instatniate Prompt 2/ Instantiate Model 3/ Call model w/prompt

(Wrong spot, do you talk about the open source models somewhere in a sidebar)

This code took me a long time to parse...

- Start with an example of usage?
- Consider marking from_file as a classmethod
- Having a template bound to a model is weird to me, Though completion does feel OK to me.
- I like active verbs, completion->complete
- Maybe it's book syntax but PromptTemplate is more descriptive to me.
- Make insert params a method on the class?
- insert-params-> doesn't feel like the operation, is resolve_params? Not sure the word for this from template languages
- insert-params, would be simpler if you went the other direction for every kwarg, string.replace in template.
- You know what, maybe I don't like this passing **instance to  Completion.create, if you just used explict param you wouldn't need this deep copy and I think the code would be clearer.
...
**ID> Don't remmeber have you introduced system prompts yet?**
...
**ID>  You're not using the insert_params() function. I prefer the format trikc you use here, use it in example 1, though you'll need to explain it..


BTW - I'm feeling stronger and stronger to just use one of them and sidebar the other one. The only reason I'd keep davinci is in case someone is using a local model, but that's probably going to be rare
_**
...
you will have one or more tuned prompts

**ID> I expected this meant in series, not at the same stage

META: Seems like I'd do someting else based on the prompt selection process, like I'd have different control flow based on what prompt was selected??

_**

you’ll want to use. A lot of the prompt
...
**ID> Basd on the current context => based on user (or system) input.
Maybe introduce the prompt input won't just be from the users, but can also be from the system somewhere earlier
_**
...
**ID> What? Never thought of this, excited to read

I think I'd open this section (and previous sections), with user examples ...

If user enters write a brief, works great.
But what if users enters write a haiku, it sucks
We need to write a new prompt
But what if we want to handle both inputs?
But how can we pick which prompt to use?
Have a prompt selection prompt ...

The term Selection Prompt confused me.

It's not  just selecting a prompt, it's selecting a prompt and  populating the paramaters. I'd almost call this partial template instantiation.

Still thinking ... Maybef there's a sidebar here on this being prompt nesting

- Figure 3.2, call out in step 2

User ask is a weird word, user_input?

_**

Selecting the right prompt for the job can seem complicated, **ID> Somehow I thought this meant designing the prompt, not selecting which prompt to use in an if check**
...
**ID> This example was very hard for me to understand ...

The nested JSON was confusing...

An alternative is to build the output as a format string, instanitating the templates as input...
Interestingly, that's what I do here.nc

https://github.com/idvorkin/nlp/blob/main/improv.py?plain=1#L227
...
while True:
    ...
    # I'd include a "Selection Prompt> at the start"
    print(response.choices[0].message.content)
    ...
    # I'd include another debug step of Prompt2"
    print(response.choices[0].message.content)
```
...
**ID> Not sure if mac folks have a clue what powershell is, is there a fruit or some such example?**
...
while True:
    ...
    # This line is weird looking...
    message = chat.completion({}).choices[0].message

    # This is weird ... Given this is history, it's odd that we haven't
    # latched the resolution of previous completion calls, but could change history in the future.
    print(f'{message.role}: {message.content}')
...
**ID>  There's something that was non-obvious to me, that  LLM workflows can be all conversational input. Maybe that comes back to my confusion above on serial vs parallel on the need of prompt selection
_**
...
**ID> This idea of prompt selection being manual is interesting ... I might start with that idea in the prompt selection section ... Then say we can have more power by letting the prompt do it
...
I don't love the word implementation layer ... But love the concept, probalby worth a picturef
...
**ID> Assuming the key problem is token size and problem with this is error rate goes up... Oh intersrint... it's almost like error propagation, and understandability and debugability goes down
...
ending up having to maintain more lines of code and a more prompts.o

**ID> Count prompts is higher,  but len of all prompts is the same.
Perhaps the problem here is actually the implemtentation becomes brittle, and expensive

Something interesting here about the fact that non-programmers can now write a program ..

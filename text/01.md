...
retrieve in the code samples. In bash, you can use the command in listing 1.1.

```bash
export OPENAI_API_KEY="sk-..."
```

*Listing 1.1: Setting the OpenAI key in bash.*

In PowerShell, you can use the command in listing 1.2.

```powershell
$Env:OPENAI_API_KEY = "sk-..."
```

*Listing 1.2: Setting the OpenAI API key in PowerShell.*
...
Let’s install it using the Python package manager as shown in listing 1.3.

```bash
pip install openai
```

*Listing 1.3: Installing the openai Python package.*

Now we have all the pieces for a first API call. Time for “Hello world”! Put the
code in listing 1.4 in `hello_world.py` and run it.
...
Running this should output something like listing 1.5.

```text
print("Hello world")
```
*Listing 1.5: Model output.*
